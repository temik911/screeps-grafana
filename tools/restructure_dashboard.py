...
STATUS_STRIP = [
    ...
    stat_panel(102, 'CPU bucket', 'Bucket = CPU savings buffer (max 10000). <2000 = burning into reserve. <500 = critical.',
               'aliasByNode(stats.gauges.$shard.cpu.bucket, 3)',
    ...
    stat_panel(103, 'GCL', 'Global Control Level — caps the number of rooms we can own.',
               'aliasByNode(stats.gauges.$shard.gcl.level, 3)',
    ...
    stat_panel(104, 'Hostiles (colony+remotes)',
               ...
               'sumSeries(stats.gauges.$shard.rooms.*.hostiles, stats.gauges.$shard.remotes.*.hostiles)',
    ...
    stat_panel(105, 'Net energy / tick',
               ...
               "alias(summarize(stats.gauges.$shard.energyFlow.net, '1min', 'avg'), 'net e/tick')",
    ...
    stat_panel(106, 'Spawn busy %',
               ...
               'averageSeries(stats.gauges.$shard.rooms.*.spawnBusyPct)',
...
ROWS = [
    ...
    ('Per-room economy', [8, 9, 61, 62, 10, 11, 25, 26, 45, 241, 46, 29], False),
    ('Energy & build', [15, 214, 215], False),
    ('Construction', [239, 240], False),
    ('Movement & Pathfind', [64, 65, 66, 24, 30, 31, 32, 217, 218, 220, 242, 243], False),
    ('Logistics', [67, 68, 69, 33, 34, 47, 48, 50], False),
    ('Logistics broker', [70, 71, 72], False),
    ('Remote mining', [16, 17, 19, 36, 301, 303, 302], False),
